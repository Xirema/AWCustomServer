...
namespace {
  ...
  (
   ...
   uint64_t packId
    ...
    auto textResourceInsertStatement = connection.prepare_statement
      (R"SQL(
	insert into
  ...
  void submitImageResources(std::vector<rTypes::ImageResource> imageResources, mysql::tcp_ssl_connection& connection, uint64_t packId) {
    auto imageResourceInsertStatement = connection.prepare_statement
      (R"SQL(
	insert into
  ...
  uint64_t submitPackMetadata(rTypes::PackMetadata const& metadata, mysql::tcp_ssl_connection& connection) {
    auto expireOldPacks = connection.prepare_statement
      (R"SQL(
	update
	  RESOURCE.PACK
	set
    ...
    auto insertNewPackMetadata = connection.prepare_statement
      (R"SQL(
	insert into 
	  RESOURCE.PACK
  ...
  rTypes::PackMetadata get_metadata_impl(net::HTTPHeaders const& headers, mysql::tcp_ssl_connection& connection) {
    ...
    auto getMetadataStatement = connection.prepare_statement
      (R"SQL(
	select 
	  *
	from
