...
namespace
{
  ...
  void submitImageResources2(std::vector<rTypes::ImageResource> imageResources, mysql::tcp_ssl_connection &connection, uint64_t packId)
  {
    auto imageResourceInsertStatement = connection.prepare_statement(R"SQL(
      insert into
        RESOURCE.IMAGE_RESOURCE
...
std::string rest::resource::upload_pack2(net::HTTPHeaders const &headers, std::string body)
{
  try
  {
    ...
    std::ranges::transform(body, data.begin(), [](char c)
                           { return static_cast<uint8_t>(c); });
    return upload_pack2_impl(headers, data);
  }
  catch (std::runtime_error const &e)
  {
    throw net::RestError("There was a problem parsing the Resource Pack: " + std::string(e.what()), net::RestError::Type::INVALID_DATA);
