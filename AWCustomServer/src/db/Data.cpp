...
void submitUnitData(std::vector<dTypes::UnitType> const& unitData, mysql::tcp_ssl_connection& connection, uint64_t mod_id) {
  ...
  for (auto const& unitType : unitData) {
    mysql::results results;
    connection.execute(unitInsertStatement.bind(mod_id, unitType.name, unitType.cost, unitType.maxFuel, unitType.maxAmmo, unitType.visionRange,
                                                unitType.movementClass, unitType.movementRange, unitType.fuelPerDay, unitType.fuelPerDayStealth,
                                                unitType.supplyRepair, unitType.transportCapacity, unitType.hitPoints, unitType.captureSpeed,
                                                unitType.ignoresVisionOcclusion, unitType.stealthType, unitType.stationaryFire),
                       results);
...
void submitWeaponData(std::vector<dTypes::WeaponType> const& weaponData, mysql::tcp_ssl_connection& connection, uint64_t mod_id) {
  ...
  for (auto const& weaponType : weaponData) {
    mysql::results results;
    connection.execute(
        weaponInsertStatement.bind(mod_id, weaponType.name, weaponType.ammoConsumed, weaponType.maxRange, weaponType.minRange, weaponType.selfTarget,
                                   weaponType.affectedByLuck, weaponType.nonLethal, weaponType.areaOfEffect, weaponType.flatDamage),
        results);
...
void submitTerrainData(std::vector<dTypes::TerrainType> const& terrainData, mysql::tcp_ssl_connection& connection, uint64_t mod_id) {
  ...
  for (auto const& terrainType : terrainData) {
    mysql::results results;
    connection.execute(
        terrainInsertStatement.bind(mod_id, terrainType.name, terrainType.stars, terrainType.maxCapturePoints, terrainType.sameAs, terrainType.income,
                                    terrainType.repair, terrainType.occludesVision, terrainType.hitPoints, terrainType.destroyed,
                                    terrainType.damagedLike, terrainType.activationMax, terrainType.activationChange, terrainType.loseIfCaptured,
                                    terrainType.loseIfAllCaptured, terrainType.destroyedOrientation),
        results);
    ...
    auto bindEffects = [&connection, &mod_id, &terrainType, &terrainActivateEffectInsertStatement](std::vector<std::string> const& effectList,
...
void submitMovementData(std::vector<dTypes::MovementClass> const& movementData, mysql::tcp_ssl_connection& connection, uint64_t mod_id) {
  ...
  auto movementRuleReferenceInsertStatement = connection.prepare_statement(R"SQL(
        insert into
          DATA.MOVEMENT_RULE_REFERENCE
          (
            MOD_ID, MOVEMENT_NAME, RULE_NAME, `ORDER`
          )
        values
          (
            ?, ?, ?, ?
...
void submitMovementRuleData(std::vector<dTypes::MovementRule> const& movementRules, mysql::tcp_ssl_connection& connection, uint64_t mod_id) {
  auto movementRuleInsertStatement = connection.prepare_statement(R"SQL(
        insert into
          DATA.MOVEMENT_RULE
          (
            MOD_ID, NAME, TYPE, MAXREPEAT, CONFIRMTYPE, STOPIFUSED
          )
        values
          (
            ?, ?, ?, ?, ?, ?
...
void submitCommanderData(std::vector<dTypes::CommanderType> const& commanderData, mysql::tcp_ssl_connection& connection, uint64_t mod_id) {
  ...
  for (auto const& commanderType : commanderData) {
    mysql::results results;
    connection.execute(commanderInsertStatement.bind(mod_id, commanderType.name, commanderType.copCost, commanderType.scopCost,
                                                     commanderType.coMeterMultiplier, commanderType.playable),
                       results);

    auto insertEffects = [&connection, &commanderType, mod_id, &effectInsertStatement](std::vector<std::string> const& effects,
                                                                                       std::string_view effectType, int effectCategory) {
      for (auto const& effect : effects) {
        mysql::results results;
        connection.execute(effectInsertStatement.bind(mod_id, commanderType.name, effect, effectType, effectCategory), results);
      }
    };
    if (commanderType.passiveUnitEffectsD2d) insertEffects(*commanderType.passiveUnitEffectsD2d, "PUE", 0);
    if (commanderType.passiveTerrainEffectsD2d) insertEffects(*commanderType.passiveTerrainEffectsD2d, "PTE", 0);
    if (commanderType.passiveGlobalEffectsD2d) insertEffects(*commanderType.passiveGlobalEffectsD2d, "PGE", 0);

    if (commanderType.passiveUnitEffectsCop) insertEffects(*commanderType.passiveUnitEffectsCop, "PUE", 1);
    if (commanderType.activeUnitEffectsCop) insertEffects(*commanderType.activeUnitEffectsCop, "AUE", 1);
    if (commanderType.passiveTerrainEffectsCop) insertEffects(*commanderType.passiveTerrainEffectsCop, "PTE", 1);
    if (commanderType.activeTerrainEffectsCop) insertEffects(*commanderType.activeTerrainEffectsCop, "ATE", 1);
    if (commanderType.passiveGlobalEffectsCop) insertEffects(*commanderType.passiveGlobalEffectsCop, "PGE", 1);
    if (commanderType.activeGlobalEffectsCop) insertEffects(*commanderType.activeGlobalEffectsCop, "AGE", 1);

    if (commanderType.passiveUnitEffectsScop) insertEffects(*commanderType.passiveUnitEffectsScop, "PUE", 2);
    if (commanderType.activeUnitEffectsScop) insertEffects(*commanderType.activeUnitEffectsScop, "AUE", 2);
    if (commanderType.passiveTerrainEffectsScop) insertEffects(*commanderType.passiveTerrainEffectsScop, "PTE", 2);
    if (commanderType.activeTerrainEffectsScop) insertEffects(*commanderType.activeTerrainEffectsScop, "ATE", 2);
    if (commanderType.passiveGlobalEffectsScop) insertEffects(*commanderType.passiveGlobalEffectsScop, "PGE", 2);
    if (commanderType.activeGlobalEffectsScop) insertEffects(*commanderType.activeGlobalEffectsScop, "AGE", 2);
  }
}
...
void submitPlayerData(std::vector<dTypes::PlayerType> const& playerData, mysql::tcp_ssl_connection& connection, uint64_t mod_id) {
  ...
  for (auto const& playerType : playerData) {
    mysql::results results;
    connection.execute(playerInsertStatement.bind(mod_id, playerType.name, playerType.commanderTypeMod, playerType.teamName), results);
...
void insertEffectTargets(mysql::tcp_ssl_connection& connection, uint64_t mod_id, std::string_view effectName, std::string_view effectType,
                         std::vector<std::string> const& targetArray) {
...
void insertEffectAffects(mysql::tcp_ssl_connection& connection, uint64_t mod_id, std::string_view effectName, std::string_view effectType,
                         std::vector<std::string> const& affectArray) {
  auto effectTargetInsertStatement = connection.prepare_statement(R"SQL(
                        insert into
...
void insertEffectUnitTypeRequired(mysql::tcp_ssl_connection& connection, uint64_t mod_id, std::string_view effectName, std::string_view effectType,
                                  std::vector<std::string> const& unitTypeArray) {
...
void insertEffectClassification(mysql::tcp_ssl_connection& connection, uint64_t mod_id, std::string_view effectName, std::string_view effectType,
                                std::vector<std::string> const& classificationArray) {
...
void insertEffectTerrainRequired(mysql::tcp_ssl_connection& connection, uint64_t mod_id, std::string_view effectName, std::string_view effectType,
                                 std::vector<std::string> const& terrainArray) {
  auto insertStatement = connection.prepare_statement(R"SQL(
                        insert into
...
void submitPassiveUnitEffectData(std::vector<dTypes::PassiveUnitEffect> const& passiveUnitEffectData, mysql::tcp_ssl_connection& connection,
                                 uint64_t mod_id) {
  ...
  for (auto const& pue : passiveUnitEffectData) {
    mysql::results results;
    connection.execute(
        passiveUnitEffectInsertStatement.bind(mod_id, pue.name, pue.firepowerMod, pue.defenseMod, pue.indirectDefenseMod, pue.minRangeMod,
                                              pue.maxRangeMod, pue.fuelUseMod, pue.ammoUseMod, pue.goodLuckMod, pue.badLuckMod,

                                              pue.movementMod, pue.visionMod, pue.terrainStarsMod, pue.terrainStarsFlatMod, pue.terrainStarsDefense,
                                              pue.terrainStarsFirepower, pue.counterfireMod, pue.counterFirst,

                                              pue.captureRateMod, pue.unitCostMod, pue.firepowerFromFunds, pue.defenseFromFunds, pue.fundsFromDamage,
                                              pue.coMeterChargeFromDealtDamage,

                                              pue.coMeterChargeFromReceivedDamage, pue.terrainStarsFlatDefense, pue.terrainStarsFlatFirepower),
        results);
...
void submitActiveUnitEffectData(std::vector<dTypes::ActiveUnitEffect> const& activeUnitEffectData, mysql::tcp_ssl_connection& connection,
                                uint64_t mod_id) {
  ...
  for (auto const& aue : activeUnitEffectData) {
    mysql::results results;
    connection.execute(
        activeUnitEffectInsertStatement.bind(mod_id, aue.name, aue.hitPointMod, aue.roundHitPoints, aue.setFuel, aue.setAmmo, aue.addFuel,
                                             aue.addAmmo, aue.multiplyFuel, aue.multiplyAmmo, aue.makeActive, aue.stunDuration, aue.coChargeFactor),
        results);
...
void submitPassiveTerrainEffectData(std::vector<dTypes::PassiveTerrainEffect> const& passiveTerrainEffectData, mysql::tcp_ssl_connection& connection,
                                    uint64_t mod_id) {
  ...
  for (auto const& pte : passiveTerrainEffectData) {
    mysql::results results;
    connection.execute(passiveTerrainEffectInsertStatement.bind(mod_id, pte.name, pte.incomeMod, pte.incomeFlatMod, pte.repairMod,
                                                                pte.occludesVisionMod, pte.visionModBoost, pte.buildCostMod),
                       results);
...
void submitActiveTerrainEffectData(std::vector<dTypes::ActiveTerrainEffect> const& activeTerrainEffectData, mysql::tcp_ssl_connection& connection,
                                   uint64_t mod_id) {
  ...
  for (auto const& ate : activeTerrainEffectData) {
    mysql::results results;
    connection.execute(
        activeTerrainEffectInsertStatement.bind(mod_id, ate.name, ate.unitSummonedName, ate.unitSummonedInitialDamage, ate.unitSummonedActive),
        results);
...
void submitPassiveGlobalEffectData(std::vector<dTypes::PassiveGlobalEffect> const& passiveGlobalEffectData, mysql::tcp_ssl_connection& connection,
                                   uint64_t mod_id) {
  ...
  for (auto const& pge : passiveGlobalEffectData) {
    mysql::results results;
    connection.execute(passiveGlobalEffectInsertStatement.bind(mod_id, pge.name, pge.variantMod, pge.movementClassVariantReplace,
                                                               pge.movementClassVariantOverride, pge.minimumVisionMod),
                       results);
...
void submitActiveGlobalEffectData(std::vector<dTypes::ActiveGlobalEffect> const& activeGlobalEffectData, mysql::tcp_ssl_connection& connection,
                                  uint64_t mod_id) {
  ...
  for (auto const& age : activeGlobalEffectData) {
    mysql::results results;
    connection.execute(activeGlobalEffectInsertStatement.bind(mod_id, age.name, age.fundMod, age.fundFlatMod, age.powerBarMod, age.powerBarPerFunds,
                                                              age.missileCount, age.missileDamage, age.missileAreaOfEffect, age.missileStunDuration,
                                                              age.coChargeFactor),
                       results);
...
void submitDefaultSettingsData(std::vector<dTypes::Settings> const& defaultSettingsData, mysql::tcp_ssl_connection& connection, uint64_t mod_id) {
  ...
  for (auto const& settings : defaultSettingsData) {
    mysql::results results;
    connection.execute(
        defaultSettingsInsertStatement.bind(mod_id, settings.name, settings.fogOfWar, settings.coPowers, settings.teams, settings.startingFunds,
                                            settings.incomeMultiplier, settings.unitLimit, settings.captureLimit, settings.dayLimit,
                                            settings.coMeterSize, settings.coMeterMultiplier),
        results);
...
void submitConfig(dTypes::Config const& config, mysql::tcp_ssl_connection& connection, uint64_t mod_id) {
  ...
  mysql::results results;
  connection.execute(insertStatement.bind(mod_id, config.minTerrainStars, config.unlimitedUnload, config.terrainDefenseScalesWithHitpoints,
...
bool upload_mod(dTypes::ModData const& modData) {
  try {
    ...
    submitMovementRuleData(modData.movementRules, session.connection, modId);
...
mysql::results getLookup(mysql::tcp_ssl_connection& connection, std::string_view tableName, int64_t modId,
  ...
  mysql::results ret;
  connection.execute(statement.bind(parameters.cbegin(), parameters.cend()), ret);
  return ret;
}

mysql::results getReferenceLookup(mysql::tcp_ssl_connection& connection, std::string_view referenceTableName, std::string_view referenceColumn,
                                  int64_t modId, std::optional<std::string_view> ownerNameFilter = {},
                                  std::optional<std::string_view> effectReferenceFilter = {}, std::optional<std::string_view> sortColumn = {}) {
  std::string sql = R"SQL(
			select 
				T.* 
			from 
...
void assignTerrainBuildRepairData(mysql::row_view const& row, dTypes::TerrainType& terrain) {
  ...
  if (type == 2) {
    if (!terrain.repairList) terrain.repairList.emplace();
    terrain.repairList->push_back(std::move(name));
  } else if (type == 1) {
    if (!terrain.buildList) terrain.buildList.emplace();
...
void assignTerrainActivationListData(mysql::row_view const& row, dTypes::TerrainType& terrain) {
  ...
  if (!terrain.activateList) terrain.activateList.emplace();
...
void assignTerrainActivationEffectData(mysql::row_view const& row, dTypes::TerrainType& terrain) {
  ...
  auto getEffectList = [&terrain, &effectType]() -> std::optional<std::vector<std::string>>& {
    if (effectType == "AUE") return terrain.activateActiveUnitEffects;
    if (effectType == "ATE") return terrain.activateActiveTerrainEffects;
    if (effectType == "AGE") return terrain.activateActiveGlobalEffects;
  ...
  if (!effectList) effectList.emplace();
  effectList->push_back(effectName);
}

void assignMovementData(mysql::row_view const& row, dTypes::MovementClass& movement) { movement.name = *get<std::string>(row[2]); }
...
void assignMovementCostData(mysql::row_view const& row, dTypes::MovementClass& movement) {
  ...
  if (variant) {
    if (!movement.variantMods) movement.variantMods.emplace();
...
void assignMovementRuleData(mysql::row_view const& row, dTypes::MovementRule& movementRule) {
  ...
  movementRule.confirmType = *get<std::string>(row[5]);
...
void getEffectTargets(mysql::tcp_ssl_connection& connection, T& effect, int64_t modId, std::string_view effectTypeCode) {
  auto results = getReferenceLookup(connection, "EFFECT_TARGET_REFERENCE", "EFFECT_NAME", modId, effect.name, effectTypeCode);
  if (results.size() > 0) {
...
void getEffectAffects(mysql::tcp_ssl_connection& connection, T& effect, int64_t modId, std::string_view effectTypeCode) {
  auto results = getReferenceLookup(connection, "EFFECT_AFFECT_REFERENCE", "EFFECT_NAME", modId, effect.name, effectTypeCode);
  if (results.size() > 0) {
...
void getEffectClassifications(mysql::tcp_ssl_connection& connection, T& effect, int64_t modId, std::string_view effectTypeCode) {
  auto results = getReferenceLookup(connection, "EFFECT_CLASSIFICATION_REFERENCE", "EFFECT_NAME", modId, effect.name, effectTypeCode);
  if (results.size() > 0) {
...
void getEffectTerrainRequired(mysql::tcp_ssl_connection& connection, T& effect, int64_t modId, std::string_view effectTypeCode) {
  auto results = getReferenceLookup(connection, "EFFECT_TERRAIN_REFERENCE", "EFFECT_NAME", modId, effect.name, effectTypeCode);
  if (results.size() > 0) {
...
void getEffectUnitRequired(mysql::tcp_ssl_connection& connection, T& effect, int64_t modId, std::string_view effectTypeCode) {
  auto results = getReferenceLookup(connection, "EFFECT_UNITTYPEREQUIRED_REFERENCE", "EFFECT_NAME", modId, effect.name, effectTypeCode);
  if (results.size() > 0) {
...
void assignPUEIntelData(mysql::row_view const& row, dTypes::PassiveUnitEffect& effect) {
  ...
  auto selector = [&](std::string const& intelType) -> std::optional<std::vector<std::string>>& {
    if (intelType == "hiddenHitPoints") return effect.hiddenHitPoints;
    if (intelType == "luckPointsVisible") return effect.luckPointsVisible;
    if (intelType == "hpPartVisible") return effect.hpPartVisible;
...
void assignCommanderEffectData(mysql::row_view const& row, dTypes::CommanderType& commander) {
  ...
  auto selector = [&]() -> std::optional<std::vector<std::string>>& {
    if (effectType == "PUE" && effectCategory == 0) return commander.passiveUnitEffectsD2d;
    if (effectType == "PUE" && effectCategory == 1) return commander.passiveUnitEffectsCop;
    if (effectType == "PUE" && effectCategory == 2) return commander.passiveUnitEffectsScop;
    if (effectType == "AUE" && effectCategory == 1) return commander.activeUnitEffectsCop;
    if (effectType == "AUE" && effectCategory == 2) return commander.activeUnitEffectsScop;
    if (effectType == "PTE" && effectCategory == 0) return commander.passiveTerrainEffectsD2d;
    if (effectType == "PTE" && effectCategory == 1) return commander.passiveTerrainEffectsCop;
    if (effectType == "PTE" && effectCategory == 2) return commander.passiveTerrainEffectsScop;
    if (effectType == "ATE" && effectCategory == 1) return commander.activeTerrainEffectsCop;
    if (effectType == "ATE" && effectCategory == 2) return commander.activeTerrainEffectsScop;
    if (effectType == "PGE" && effectCategory == 0) return commander.passiveGlobalEffectsD2d;
    if (effectType == "PGE" && effectCategory == 1) return commander.passiveGlobalEffectsCop;
    if (effectType == "PGE" && effectCategory == 2) return commander.passiveGlobalEffectsScop;
    if (effectType == "AGE" && effectCategory == 1) return commander.activeGlobalEffectsCop;
    if (effectType == "AGE" && effectCategory == 2) return commander.activeGlobalEffectsScop;
...
std::vector<dTypes::WeaponType> get_weapons(int64_t modId, std::optional<std::string_view> const& weaponName) {
  sqlutil::Session session;
  auto results = getLookup(session.connection, "WEAPON_TYPE", modId, weaponName);
  std::vector<dTypes::WeaponType> weapons;
  for (auto const& row : results.rows()) {
    auto& newWeapon = weapons.emplace_back();
    assignWeaponData(row, newWeapon);
    auto baseDamageResults = getReferenceLookup(session.connection, "WEAPON_BASEDAMAGE_REFERENCE", "WEAPONTYPE_NAME", modId, newWeapon.name);
    ...
    auto stealthTargetsResults = getReferenceLookup(session.connection, "WEAPON_STEALTHTARGET_REFERENCE", "WEAPONTYPE_NAME", modId, newWeapon.name);
...
std::vector<dTypes::UnitType> get_units(int64_t modId, std::optional<std::string_view> const& unitName) {
  ...
  for (auto const& row : results.rows()) {
    ...
    auto weaponResults = getReferenceLookup(session.connection, "UNIT_WEAPON_REFERENCE", "UNITTYPE_NAME", modId, newUnit.name, {}, "WEAPONORDER");
    ...
    auto classificationResults = getReferenceLookup(session.connection, "UNIT_CLASSIFICATION_REFERENCE", "UNITTYPE_NAME", modId, newUnit.name);
    for (auto const& classificationRow : classificationResults.rows()) {
      assignUnitClassificationData(classificationRow, newUnit);
    }
    auto transportListResults = getReferenceLookup(session.connection, "UNIT_TRANSPORTLIST_REFERENCE", "UNITTYPE_NAME", modId, newUnit.name);
...
std::vector<dTypes::TerrainType> get_terrains(int64_t modId, std::optional<std::string_view> const& terrainName) {
  ...
  auto results = getLookup(session.connection, "TERRAIN_TYPE", modId, terrainName);
  for (auto const& row : results.rows()) {
    auto& newTerrain = terrains.emplace_back();
    assignTerrainData(row, newTerrain);
    auto buildRepairResults =
        getReferenceLookup(session.connection, "TERRAIN_BUILDREPAIR_REFERENCE", "TERRAINTYPE_NAME", modId, newTerrain.name, {}, "UNITORDER");
    for (auto const& buildRepairRow : buildRepairResults.rows()) {
      assignTerrainBuildRepairData(buildRepairRow, newTerrain);
    }

    auto activateListResults = getReferenceLookup(session.connection, "TERRAIN_ACTIVATIONLIST_REFERENCE", "TERRAINTYPE_NAME", modId, newTerrain.name);
    for (auto const& activateListRow : activateListResults.rows()) {
      assignTerrainActivationListData(activateListRow, newTerrain);
    }

    auto activateEffectResults =
        getReferenceLookup(session.connection, "TERRAIN_ACTIVATIONEFFECT_REFERENCE", "TERRAINTYPE_NAME", modId, newTerrain.name);
...
std::vector<dTypes::CommanderType> get_commanders(int64_t modId, std::optional<std::string_view> const& commanderName) {
  ...
  auto results = getLookup(session.connection, "COMMANDER_TYPE", modId, commanderName);
  for (auto const& row : results.rows()) {
    auto& newCommander = commanders.emplace_back();
    assignCommanderData(row, newCommander);
    auto commanderEffectResults = getReferenceLookup(session.connection, "COMMANDER_EFFECT_REFERENCE", "COMMANDER_NAME", modId, newCommander.name);
...
std::vector<dTypes::MovementClass> get_movements(int64_t modId, std::optional<std::string_view> const& movementName) {
  ...
  auto results = getLookup(session.connection, "MOVEMENT_TYPE", modId, movementName);
  for (auto const& row : results.rows()) {
    auto& newMovement = movements.emplace_back();
    assignMovementData(row, newMovement);
    auto movementCostResults = getReferenceLookup(session.connection, "MOVEMENT_COST_REFERENCE", "MOVEMENT_NAME", modId, newMovement.name);
    for (auto const& movementCostRow : movementCostResults.rows()) {
      assignMovementCostData(movementCostRow, newMovement);
    }
    auto movementRuleResults = getReferenceLookup(session.connection, "MOVEMENT_RULE_REFERENCE", "MOVEMENT_NAME", modId, newMovement.name);
...
std::vector<dTypes::MovementRule> get_movement_rules(int64_t modId, std::optional<std::string_view> const& ruleName) {
  ...
  auto results = getLookup(session.connection, "MOVEMENT_RULE", modId, ruleName);
...
std::vector<dTypes::PlayerType> get_players(int64_t modId, std::optional<std::string_view> const& playerName) {
  ...
  auto results = getLookup(session.connection, "PLAYER_TYPE", modId, playerName);
  for (auto const& row : results.rows()) {
    auto& newPlayer = players.emplace_back();
    assignPlayerData(row, newPlayer);
    auto permittedPlayerSlotsResults =
        getReferenceLookup(session.connection, "PLAYER_PERMITTEDPLAYERSLOT_REFERENCE", "PLAYERTYPE_NAME", modId, newPlayer.name);
    ...
    auto permittedCommanderResults =
        getReferenceLookup(session.connection, "PLAYER_PERMITTEDCOMMANDERTYPE_REFERENCE", "PLAYERTYPE_NAME", modId, newPlayer.name);
...
std::vector<dTypes::PassiveUnitEffect> get_pues(int64_t modId, std::optional<std::string_view> const& effectName) {
  ...
  auto results = getLookup(session.connection, "PASSIVE_UNIT_EFFECT", modId, effectName);
  for (auto const& row : results.rows()) {
    ...
    auto firepowerTerrainResults = getReferenceLookup(session.connection, "PUE_FIREPOWERFROMTERRAIN_REFERENCE", "EFFECT_NAME", modId, newEffect.name);
    ...
    auto defenseTerrainResults = getReferenceLookup(session.connection, "PUE_DEFENSEFROMTERRAIN_REFERENCE", "EFFECT_NAME", modId, newEffect.name);
    ...
    auto visionVariantResults = getReferenceLookup(session.connection, "PUE_VISIONVARIANT_REFERENCE", "EFFECT_NAME", modId, newEffect.name);
    ...
    auto firepowerVariantResults = getReferenceLookup(session.connection, "PUE_FIREPOWERVARIANT_REFERENCE", "EFFECT_NAME", modId, newEffect.name);
    ...
    auto defenseVariantResults = getReferenceLookup(session.connection, "PUE_DEFENSEVARIANT_REFERENCE", "EFFECT_NAME", modId, newEffect.name);
    ...
    auto intelResults = getReferenceLookup(session.connection, "PUE_INTEL_REFERENCE", "EFFECT_NAME", modId, newEffect.name);
...
std::vector<dTypes::ActiveUnitEffect> get_aues(int64_t modId, std::optional<std::string_view> const& effectName) {
  ...
  auto results = getLookup(session.connection, "ACTIVE_UNIT_EFFECT", modId, effectName);
  for (auto const& row : results.rows()) {
    auto& newEffect = effects.emplace_back();
...
std::vector<dTypes::PassiveTerrainEffect> get_ptes(int64_t modId, std::optional<std::string_view> const& effectName) {
  ...
  auto results = getLookup(session.connection, "PASSIVE_TERRAIN_EFFECT", modId, effectName);
  for (auto const& row : results.rows()) {
    auto& newEffect = effects.emplace_back();
    ...
    auto buildListResults = getReferenceLookup(session.connection, "PTE_BUILDLISTMOD_REFERENCE", "EFFECT_NAME", modId, newEffect.name);
...
std::vector<dTypes::ActiveTerrainEffect> get_ates(int64_t modId, std::optional<std::string_view> const& effectName) {
  ...
  auto results = getLookup(session.connection, "ACTIVE_TERRAIN_EFFECT", modId, effectName);
  for (auto const& row : results.rows()) {
    auto& newEffect = effects.emplace_back();
...
std::vector<dTypes::PassiveGlobalEffect> get_pges(int64_t modId, std::optional<std::string_view> const& effectName) {
  ...
  auto results = getLookup(session.connection, "PASSIVE_GLOBAL_EFFECT", modId, effectName);
  for (auto const& row : results.rows()) {
    auto& newEffect = effects.emplace_back();
    assignPGEData(row, newEffect);
    getEffectTargets(session.connection, newEffect, modId, "PGE");

    auto variantHintResults = getReferenceLookup(session.connection, "PGE_VARIANTHINTMOD_REFERENCE", "EFFECT_NAME", modId, newEffect.name);
...
std::vector<dTypes::ActiveGlobalEffect> get_ages(int64_t modId, std::optional<std::string_view> const& effectName) {
  ...
  auto results = getLookup(session.connection, "ACTIVE_GLOBAL_EFFECT", modId, effectName);
  for (auto const& row : results.rows()) {
    auto& newEffect = effects.emplace_back();
    assignAGEData(row, newEffect);
    getEffectTargets(session.connection, newEffect, modId, "AGE");

    auto missileTargetResults = getReferenceLookup(session.connection, "AGE_MISSILETARGETMETHOD_REFERENCE", "EFFECT_NAME", modId, newEffect.name);
...
std::vector<dTypes::Settings> get_settings(int64_t modId, std::optional<std::string_view> const& settingsName) {
  ...
  auto results = getLookup(session.connection, "DEFAULT_GAME_SETTINGS", modId, settingsName);
  for (auto const& row : results.rows()) {
    auto& newSettings = settings.emplace_back();
    assignSettingsData(row, newSettings);

    auto settingsVariantResults =
        getReferenceLookup(session.connection, "DEFAULTSETTINGS_VARIANT_REFERENCE", "SETTING_NAME", modId, newSettings.name);
...
dTypes::Config get_mod_config(int64_t modId) {
  sqlutil::Session session;
  dTypes::Config ret;
  auto results = getLookup(session.connection, "CONFIG", modId);
  for (auto const& row : results.rows()) {
...
dTypes::ModMetadata get_mod_metadata(std::optional<int64_t> const& modId, std::optional<std::string_view> const& name,
                                     std::optional<std::string_view> const& version) {
  sqlutil::Session session;
  std::string sql = R"SQL(
			select 
				*
...
std::vector<dTypes::ModMetadata> get_mods(std::optional<bool> const& showExpired, std::optional<std::string_view> const& name,
  ...
  if (author) {
    ...
    parameters.emplace_back(*author);*/
  }
  auto statement = session.connection.prepare_statement(selectSql);
  mysql::results results;
  session.connection.execute(statement.bind(parameters.begin(), parameters.end()), results);
