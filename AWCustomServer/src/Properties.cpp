...
namespace properties
{
  ...
  bool Properties::getBool(std::string const &name, std::optional<bool> defaultValue) const
  {
    ...
    {
      if (
          boost::iequals(it->second, "true") || boost::iequals(it->second, "t") || boost::iequals(it->second, "1"))
      {
        return true;
      }
      if (
          boost::iequals(it->second, "false") || boost::iequals(it->second, "f") || boost::iequals(it->second, "0"))
      {
        return false;
      }
  ...
  std::unique_ptr<bool[]> Properties::getBoolArray(std::string const &name, std::optional<std::unique_ptr<bool[]>> defaultValue) const
  {
    if (auto it = properties.find(name); it != properties.end())
    {
      ...
      {
        if (
            boost::iequals(strings[i], "true") || boost::iequals(strings[i], "t") || boost::iequals(strings[i], "1"))
        {
          ret[i] = true;
        }
        if (
            boost::iequals(strings[i], "false") || boost::iequals(strings[i], "f") || boost::iequals(strings[i], "0"))
        {
          ret[i] = false;
        }
